...
def get_data(sentences, feature_names, vocs, max_len, model='train',
              use_char_feature=False, word_len=None, trace=False):
    '''
    sentences : list of sentence
    sentence : list of feature_tokens
    feature_token : list of features
    '''
...
def init_data(path, feature_names, vocs, max_len, model='train',
    ...
    for index, sentence in enumerate(sentences):
        feature_tokens = sentence.strip().split('\n')
        for x in range(len(feature_tokens)):
          feature_tokens[x] = feature_tokens[x].strip().split(sep)
        sentences[index] = feature_tokens
    data_dict = get_data(sentences, 
                          feature_names, 
                          vocs, 
                          max_len, 
                          model,
