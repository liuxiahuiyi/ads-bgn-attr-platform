...
#    son_nodes_result = find_all_son_nodes(concept, son_set)
    ...
    if rep_e_c > 0:
      blc_list.append((concept, rep_e_c, n_c_e, sigma_n_c_ei, sigma_n_ci_e, p_e_c, p_c_e))
...
def main():
  ...
  with codecs.open('result/blc.txt', 'w', 'utf-8') as f, codecs.open('result/blc_relation.txt', 'w', 'utf-8') as f1:
    for blc in all_blc_lists:
      entity = blc[0]
      concepts_with_rep_list = blc[1]
      f.write('entity: ' + entities[entity] + '\n')
      f.write('blc: ' + concepts[concepts_with_rep_list[0][0]] + '    ' + str(concepts_with_rep_list[0][1]) + '\n')
      f.write('concepts: \n')
      for x in concepts_with_rep_list:
        f.write('    %s (%.5f: %.3f=%s/%s, %.3f=%s/%s)\n' % (concepts[x[0]], x[1], x[5], x[2], x[3], x[6], x[2], x[4]))
      f.write('\n')
