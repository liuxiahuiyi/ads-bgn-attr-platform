...
object BlcInference {
  ...
  def findBlcTopK(conceptSet: Set[(String, String, Int, Int)], k: Int, nInstance: Long) : List[Tuple3[String, String, Double]] = {
    val epsilon = 0.00001
  ...
  def main(arguments: Array[String]) {
    ...
    val getConceptNameSql = s"""
      select hash as concept_id_raw, concept as concept_name
    ...
    val wholeTable = entityConceptWeightDF.join(entityNameDF, entityConceptWeightDF("entity_id")===entityNameDF("entity_id_raw"), "left_outer")
                         ...
                         .select("entity_id", "entity_name", "concept_id", "concept_name", "sum_weight", "weight")
                         .rdd.map { x =>
                           ((x(0).toString, x(1).toString), (x(2).toString, x(3).toString, x(4).toString.toInt, x(5).toString.toInt))
                         }.groupByKey().map { x =>
                           (x._1, x._2.toSet)
                         }
...
//    println(wholeTable.take(10).foreach(println))
    val blc = wholeTable.flatMap { x =>
      ...
      val conceptSet = x._2
      var oneDimConceptSet: Set[(String, String, Int, Int)] = Set()
      conceptSet.foreach { conceptInfo =>
        if (!conceptInfo._2.contains(",")) {
          oneDimConceptSet += conceptInfo
        }
      }
      var blcSeq: Seq[Row]= Seq()
