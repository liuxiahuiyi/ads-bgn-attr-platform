...
object DeduplicateFilnalConcepts {
  def main(arguments: Array[String]) {
    val spark = SparkSession
      .builder()
      .appName("DeduplicateFilnalConcepts")
      .enableHiveSupport()
      .getOrCreate()
    import spark.implicits._

    val args = new CommonArgs(arguments)
    val log_object = new log.ConceptFromSegmentationLog()
    ...
    val filterRDD = check_concept_duplicate(logRDD, rawConceptList, rawConceptIndex)
  ...
  def check_concept_duplicate(logRDD: RDD[String], rawConceptList: Set[String],
    ...
    logRDD.filter { concept =>
      ...
      !check_sub_string_coverage(0, sub_start_end_pos, concept.length, result_set)
    }
  }

  def check_sub_string_coverage(start: Int,
    ...
    for (end <- sub_start_end_pos_dict(start)) {
      val result = check_sub_string_coverage(end+1, sub_start_end_pos_dict, string_len, result_set_)
      result_set_ += result
      //if result:
      //  break
    }
